...
**Для сборки и развёртывания бота требуется доступ к интернету** для загрузки Python пакетов из PyPI.

Если VM находится в сети университета без доступа к внешнему интернету:
- Используйте кэшированные Docker образы из harbor.pg.innopolis.university
- Или собирайте образы локально и загружайте на VM через `docker save/load`
...
**Примечание:** Скрипт требует доступа к интернету для `docker build`.
